...
class FlightManager:
    def __init__(self, airportFile, flightFile):
        ...
        for line in contents:
            ...
            for i in range(len(l)):
                l[i] = l[i].strip()
            self.flights.append(
                Flight(index, l[4], int(l[5]), self.airports[codes[l[0]]], self.airports[codes[l[1]]], l[2], l[3],
                       l[6]))
    ...
    def searchAirport(self, string):
        ...
        for airport in self.airports:
            if (len(string) <= len(airport.getCity())) and (
                    airport.getCity()[:len(string)].lower() == string.lower()) and not airport in matchingAirports:
                matchingAirports.append(airport)

            if len(string) <= len(airport.getSearchString()) and airport.getSearchString()[:len(
                    string)].lower() == string.lower() and not airport in matchingAirports:
                matchingAirports.append(airport)
